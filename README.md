...
- The current separate discovery service now ships a signed manifest contract, client-side continuity pinning for that manifest, optional signed attestation-verifier / enclave-measurement / attestation-document-hash fields plus a nonce-bound `/v1/attestation` evidence fetch and app-server-pinned freshness window, requires the attested blind-evaluation key to match the manifest-pinned OPRF key, requires the attestation payload signature to verify under the manifest issuer key, blind-evaluates local handle hashes with manifest-pinned `dleq_per_element_preview` proofs before clients finalize them into tokens, uses short-lived single-purpose discovery tickets with a small signed operation budget plus opaque bootstrap invite tokens instead of stable account IDs, and purges expired bootstrap-invite-backed token rows from the preview registry, but it remains development-only until it moves beyond `blind_evaluation_preview` / `unattested_development`.
- The signed discovery manifest now also pins `attestation_challenge_mode = nonce_b64_required_preview` whenever attestation evidence is configured, so supported clients fail closed if the preview silently downgrades from nonce-bound attestation back to a static evidence fetch.
...
- The current preview now also pins `directory_backend = simulated_enclave_preview` and `host_enclave_protocol_version = 1` in the signed discovery contract so the eventual enclave-backed replacement has a fixed compatibility boundary.
