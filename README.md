...
- The current separate discovery service now ships a signed manifest contract, client-side continuity pinning for that manifest, blind-evaluates local handle hashes into finalized tokens, and returns opaque bootstrap invite tokens instead of stable account IDs, but it remains development-only until it moves beyond `blind_evaluation_preview` / `unattested_development`.
