...
import render_audit_findings_report
...
def main() -> int:
    ...
    archive_path = Path(shutil.make_archive(str(archive_base), "zip", root_dir=bundle_dir, base_dir="."))
    archive_sha256 = write_sha256(archive_path)
    findings_path = REPO_ROOT / "docs" / "AUDIT_FINDINGS.json"
    ...
    findings_summary = render_audit_findings_report.build_report(findings_payload)
    ...
    handoff = {
        ...
        "audit_findings": findings_metadata,
    ...
    (output_dir / "audit-findings-summary.md").write_text(findings_summary, encoding="utf-8")
