...
4. Add encrypted group message transport on top of that state model.
   Status: implemented as a dedicated opaque `private-groups/messages/publish` + `private-groups/messages/fetch` transport on the current Android/web path. Messages are encrypted/authenticated in `pqmsg-core` and the server no longer computes clear recipient fanout. Fetch no longer returns clear `sender_user_id`; supported clients recover the sender locally from the signed group envelope plus the current member set. This is still not the final low-metadata design, because the server stores per-group message rows and still sees `sender_user_id` on publish ingress.
