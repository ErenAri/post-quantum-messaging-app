...
struct IosKemKeyPair {
    public_key: Vec<u8>,
    secret_key: Vec<u8>,
}

fn kem_keypair(kem: &MlKem768) -> Result<IosKemKeyPair, PqmsgIosError> {
...
fn suite_from_suite_id(suite_id: u16) -> Result<Suite, PqmsgIosError> {
    match suite_id {
...
fn decode_signing_key_b64(field: &'static str, value: &str) -> Result<SigningKey, PqmsgIosError> {
...
fn auth_signing_key_for_user(keys: &UserKeysFile) -> Result<SigningKey, PqmsgIosError> {
    let signing_key =
...
#[uniffi::export]
pub fn require_pq_backend_enabled() -> Result<(), PqmsgIosError> {
    let profile = runtime_crypto_profile()?;
...
#[uniffi::export]
pub fn suite_id_from_suite(suite: Suite) -> Result<u16, PqmsgIosError> {
...
#[uniffi::export]
pub fn suite_from_suite_id_json(suite_id: u16) -> Result<String, PqmsgIosError> {
    let suite = suite_from_suite_id(suite_id)?;
    serde_json::to_string(&suite).map_err(Into::into)
}

#[uniffi::export]
pub fn verify_identity_sig_keypair(keys_json: String) -> Result<bool, PqmsgIosError> {
...
#[uniffi::export]
pub fn load_user_profile(keys_json: String) -> Result<UserProfile, PqmsgIosError> {
    let keys = read_keys_file(&keys_json)?;
    Ok(UserProfile {
...
#[uniffi::export]
pub fn build_register_payload(keys_json: String) -> Result<RegisterPayload, PqmsgIosError> {
    let keys = read_keys_file(&keys_json)?;
    Ok(RegisterPayload {
        user_id: keys.user_id,
...
pub fn build_publish_prekeys_payload(
    keys_json: String,
) -> Result<PublishPrekeysPayload, PqmsgIosError> {
    let keys = read_keys_file(&keys_json)?;
...
) -> Result<SendResult, PqmsgIosError> {
    let mut keys = read_keys_file(&keys_json)?;
...
#[uniffi::export]
pub fn normalize_session_json(session_json: String) -> Result<String, PqmsgIosError> {
...
) -> Result<DecryptResult, PqmsgIosError> {
    let keys = read_keys_file(&keys_json)?;
...
fn read_keys_file(keys_json: &str) -> Result<UserKeysFile, PqmsgIosError> {
...
fn to_signed_prekey(keys: &UserKeysFile) -> Result<OneTimePreKey, PqmsgIosError> {
    Ok(OneTimePreKey {
...
fn to_pq_signed_prekey(keys: &UserKeysFile) -> Result<KEMPreKey, PqmsgIosError> {
    Ok(KEMPreKey {
...
fn bundle_to_core(bundle: &ServerBundle, suite: Suite) -> Result<PreKeyBundle, PqmsgIosError> {
...
fn make_ad(sender: &str, recipient: &str) -> Result<Vec<u8>, PqmsgIosError> {
    conversation_associated_data(sender, recipient).map_err(Into::into)
}

fn decode_b64(field: &'static str, value: &str) -> Result<Vec<u8>, PqmsgIosError> {
    B64.decode(value.as_bytes())
        .map_err(|_| invalid_input(format!("invalid base64 for field '{field}'")))
}

fn decode_b64_32(field: &'static str, value: &str) -> Result<[u8; 32], PqmsgIosError> {
    let bytes = decode_b64(field, value)?;
    if bytes.len() != 32 {
        return Err(invalid_input(format!(
            "field '{field}' must decode to 32 bytes (got {})",
...
fn invalid_input(message: impl Into<String>) -> PqmsgIosError {
    PqmsgIosError::InvalidInput {
        reason: message.into(),
    }
}

fn operation_failed(message: impl Into<String>) -> PqmsgIosError {
    PqmsgIosError::OperationFailed {
        reason: message.into(),
    }
}

impl From<CoreError> for PqmsgIosError {
    fn from(value: CoreError) -> Self {
        operation_failed(value.to_string())
    }
}

impl From<serde_json::Error> for PqmsgIosError {
