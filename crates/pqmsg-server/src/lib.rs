...
const MAX_PROFILE_DISPLAY_NAME_LEN: usize = 128;
...
const MAX_FILE_BLOB_BYTES: usize = 900_000;
const MAX_AVATAR_BLOB_BYTES: usize = 262_144;
const MAX_TYPING_EVENTS: i64 = 128;
const PRESENCE_TTL_SECONDS: i64 = 180;
const TYPING_TTL_SECONDS: i64 = 15;
...
struct UpsertProfileRequest {
    ...
    avatar_bytes_base64: Option<String>,
}
...
#[derive(Debug, Deserialize)]
struct TypingUpdateRequest {
    is_typing: bool,
}

#[derive(Debug, Serialize)]
struct TypingUpdateResponse {
    recipient_user_id: String,
    sender_user_id: String,
...
#[derive(Debug, Serialize)]
struct TypingInboxResponse {
    user_id: String,
    typing: Vec<TypingIndicator>,
    checked_at: String,
}
...
fn file_download_auth_message(auth: &RequestAuth, file_id: &str) -> Result<Vec<u8>, AppError> {
...
fn profile_upsert_auth_message(
    auth: &RequestAuth,
    user_id: &str,
...
fn profile_get_auth_message(auth: &RequestAuth, user_id: &str) -> Result<Vec<u8>, AppError> {
...
fn presence_update_auth_message(
    auth: &RequestAuth,
    user_id: &str,
    status: &str,
) -> Result<Vec<u8>, AppError> {
...
fn presence_get_auth_message(auth: &RequestAuth, user_id: &str) -> Result<Vec<u8>, AppError> {
...
fn typing_get_auth_message(auth: &RequestAuth, user_id: &str) -> Result<Vec<u8>, AppError> {
...
fn validate_mime_type(field: &'static str, value: &str) -> Result<String, AppError> {
    let trimmed = value.trim();
...
fn validate_optional_mime_type(
    field: &'static str,
    value: Option<&str>,
...
fn validate_optional_profile_display_name(value: Option<&str>) -> Result<Option<String>, AppError> {
    let Some(raw) = value else {
        return Ok(None);
    };
    let trimmed = raw.trim();
...
fn validate_presence_status(value: &str) -> Result<String, AppError> {
