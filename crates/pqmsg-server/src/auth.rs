...
use crate::{
    ...
    AUTH_TAG_DISCOVERY_PHONE_HASHES_HASH, AUTH_TAG_DISCOVERY_PURPOSE,
    AUTH_TAG_DISCOVERY_QUERY_HASHES_HASH, AUTH_TAG_ENDPOINT, AUTH_TAG_FILE_BLOB_HASH,
    AUTH_TAG_FILE_ID, AUTH_TAG_FILE_MIME_HASH, AUTH_TAG_FILE_RECIPIENT_ID, AUTH_TAG_GROUP_ID,
    AUTH_TAG_GROUP_MEMBERS_HASH, AUTH_TAG_GROUP_MEMBER_USER_ID, AUTH_TAG_GROUP_RECIPIENTS_HASH,
    AUTH_TAG_GROUP_SENDER_USER_ID, AUTH_TAG_LINK_DEVICE_ID, AUTH_TAG_MESSAGE_BLOB, AUTH_TAG_NONCE,
    AUTH_TAG_PREKEY_PQSPK_HASH, AUTH_TAG_PREKEY_SPK_HASH, AUTH_TAG_PRESENCE_STATUS,
    AUTH_TAG_PROFILE_AVATAR_HASH, AUTH_TAG_PROFILE_AVATAR_MIME_HASH,
    AUTH_TAG_PROFILE_DISPLAY_NAME_HASH, AUTH_TAG_PROFILE_USERNAME_HASH,
    AUTH_TAG_PROFILE_USERNAME_LOOKUP_ENABLED, AUTH_TAG_PUSH_DEVICE_ID, AUTH_TAG_PUSH_TOKEN_HASH,
    AUTH_TAG_RECIPIENT_ID, AUTH_TAG_REVOKE_DEVICE_ID, AUTH_TAG_ROTATE_CHALLENGE_ID,
    AUTH_TAG_ROTATE_NEW_PQ_SIG_HASH, AUTH_TAG_ROTATE_NEW_SIG_HASH, AUTH_TAG_ROTATE_NEW_X25519_HASH,
    AUTH_TAG_ROTATE_PQ_SIG_CURRENT_HASH, AUTH_TAG_ROTATE_PQ_SIG_NEW_HASH,
...
pub(crate) fn contact_discovery_ticket_auth_message(
    auth: &RequestAuth,
    user_id: &str,
    purpose: &str,
) -> Result<Vec<u8>, AppError> {
    let mut records = auth_common_records(auth, "contact-discovery-ticket");
    records.push(TlvRecord {
        ty: AUTH_TAG_RECIPIENT_ID,
        value: user_id.as_bytes().to_vec(),
    });
    ...
    encode(&records).map_err(|_| {
        AppError::internal("failed to encode contact-discovery-ticket auth transcript")
    })
}
...
pub(crate) fn profile_upsert_auth_message(
    ...
    hasher.update(username.unwrap_or_default().as_bytes());
    records.push(TlvRecord {
